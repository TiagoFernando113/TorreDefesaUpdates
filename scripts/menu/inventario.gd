...
func _abrir_baus(ui: CanvasLayer) -> void:
	var menu_estava_visivel : bool = m._menu_contents and m._menu_contents.visible
	...
	m._inv_lbl(pnl, "BAUS DA MOCHILA", 0, 18, pw, 36, 30, Color(1.0, 0.78, 0.18), HORIZONTAL_ALIGNMENT_CENTER)
	m._inv_lbl(pnl, "Ganhos nas waves. Abra para receber ouro, cristais, skins, comandantes, habilidades e consumíveis.", 28, 56, pw - 56, 28, 16,
		Color(0.72, 0.86, 1.0, 0.78), HORIZONTAL_ALIGNMENT_CENTER)
	var res_lbl: Label = m._inv_lbl(pnl, "Selecione um bau para abrir.", 34, ph - 142.0, pw - 68.0, 72, 17,
	...
	var _build_rewards := func(recompensas: Array) -> void:
		...
		for ri in range(recompensas.size()):
			...
			var card_r: Panel = m._inv_painel(rewards_cont, float(ri) * (r_w + r_gap), 0.0, r_w, r_h,
			...
			var sprite_key_r: String = m._item_sprite_key_from_item(item)
			badge.draw.connect(func():
				if sprite_key_r != "" and m._draw_item_sprite(badge, sprite_key_r, Rect2(2.0, 2.0, 34.0, 34.0)):
					return
			...
			m._inv_lbl(card_r, nome_r, 50, 9, r_w - 58, 24, 13, Color(0.92, 0.94, 1.0), HORIZONTAL_ALIGNMENT_LEFT)
			var tipo_legenda : String = "Cartas" if tipo == "pet_card" else ("Comandante" if tipo == "pet" else tipo.capitalize())
			m._inv_lbl(card_r, "%s  x%d" % [tipo_legenda, qtd_r], 50, 32, r_w - 58, 20, 12,
	...
	for i in range(tiers.size()):
		...
		var info: Dictionary = m._bau_info(tid)
		...
		var title: Label = m._inv_lbl(card, str(info["nome"]), 8, 12, card_w - 16, 26, 22 if cols == 4 else 18,
		...
		m._inv_lbl(card, str(info["sub"]).to_upper(), 8, 40, card_w - 16, 22, 15,
		...
		icon.draw.connect(func():
			...
			m._draw_bau_icon(icon, tid, int(Salvar.baus_estoque.get(tid, 0)), abertura_prog)
		)
		card_controls.append(icon)
		var count_lbl: Label = m._inv_lbl(card, "", 8, card_h - 56, card_w - 16, 28, 24,
		...
		card.pressed.connect(func():
			...
			m.get_tree().create_timer(0.62, true).timeout.connect(func():
	...
	fechar.pressed.connect(func():
		ov.queue_free()
		if menu_estava_visivel and m._menu_contents:
