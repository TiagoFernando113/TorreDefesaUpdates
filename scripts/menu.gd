...
func _construir_ui() -> void :
	...
	btn_rank.pressed.connect( func():
		if not (_mod_ranking and _mod_ranking._ranking_overlay) and not _config_overlay and not (_mod_loja and _mod_loja._loja_overlay) and not _talentos_overlay:
			_abrir_ranking(ui))
...
func _on_perfil_input(event: InputEvent) -> void :
	if _is_primary_press(event):
		if (
			not _perfil_overlay and not _config_overlay and not (_mod_loja and _mod_loja._loja_overlay)
